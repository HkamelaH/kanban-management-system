...
namespace IntroSE.Kanban.Backend.DataAccessLayer.DAL
{
    public class DALcolumncontroller : DALcontroller
    {
        ...
        protected override DAOs ConvertReaderToObject(SQLiteDataReader reader)
        {
            ...
            ColumnDAO result = new ColumnDAO(id, intValue, stringValue, anotherIntValue);
            
            return result;
        }

        public List<ColumnDAO> allcolumns()
        {
            return Select().Cast<ColumnDAO>().ToList();
        }
        ...
        /// </summary>
        /// <param name="column"> column dao to add its fields to the table</param>
        /// <returns></returns>
        public bool Insert(ColumnDAO column)
        {
            using (var connections = new SQLiteConnection(connection))
            {
                ...
                {
                    connections.Open();
                    command.CommandText = $"INSERT INTO {ColumnTable}({ColumnDAO.Column_BoardId},{ColumnDAO.Column_Ordinal},{ColumnDAO.Column_Name},{ColumnDAO.Column_TaskLimit})"
