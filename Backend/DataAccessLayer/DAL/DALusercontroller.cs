...
namespace IntroSE.Kanban.Backend.DataAccessLayer.DAL
{
    public class DALusercontroller : DALcontroller
    {
        ...
        protected override DTOs ConvertReaderToObject(SQLiteDataReader reader)
        {
            ...
            UserDTO result = new UserDTO(username, password, isAdmin);
            return result;
        }
        ...
        public List<UserDTO> AllTheUsers()
        {
		...
		public bool InsertUser(UserDTO user)
        {
            using (var connections = new SQLiteConnection(connection))
            {
                ...
                try
                {
                    ...
                    SQLiteParameter P1 = new SQLiteParameter(@"email", user.Email);
                    SQLiteParameter P2 = new SQLiteParameter(@"password", user.Password);
